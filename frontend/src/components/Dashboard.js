...
const Dashboard = () => {
  ...
  const handleRecommend = async (e) => {
    ...
    try {
      ...
      // Update notifications based on recommendation
      const newNotifs = [
        { id: Date.now(), type: 'info', message: `Optimal time for ${data.crop} planting in ${data.season} season.`, time: 'Just now' },
        { id: Date.now() + 1, type: 'warn', message: `Ensure soil pH stays near ${formData.ph} for best ${data.crop} yield.`, time: 'Just now' }
      ];
  ...
  const handleDownloadReport = () => {
    ...
    const reportContent = `SMART CROP RECOMMENDATION REPORT
================================
...
RECOMMENDATION:
Crop: ${recommendation.crop} ${recommendation.emoji}
Season: ${recommendation.season}
Fertilizer (NPK): ${recommendation.fertilizer}
Watering Schedule: ${recommendation.watering}

MARKET INTELLIGENCE:
Expected Price: ${recommendation.market.price}
Market Trend: ${recommendation.market.trend}

MANAGEMENT PLAN:
${recommendation.insights.map(i => '- ' + i).join('\n')}

Generated by Patel & Co. Smart Farming Hub
`;
    ...
    a.href = url;
    a.download = `Crop_Report_${recommendation.crop}.txt`;
                    ...
                    <div className="lg:col-span-1 bg-[#064e3b]/40 rounded-3xl p-8 border border-emerald-900/30 shadow-xl backdrop-blur-sm">
                      ...
                      <form onSubmit={handleRecommend} className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div>
                            <label className="block text-sm font-semibold text-slate-400 mb-2">Nitrogen (N)</label>
                            <input
                              type="number"
                          ...
                          <div>
                            <label className="block text-sm font-semibold text-slate-400 mb-2">Phosphorus (P)</label>
                            <input
                              type="number"
                              className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-emerald-500"
                              value={formData.P}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div>
                            <label className="block text-sm font-semibold text-slate-400 mb-2">Potassium (K)</label>
                            <input
                              type="number"
                              className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-emerald-500"
                              value={formData.K}
                          ...
                          <div>
                            <label className="block text-sm font-semibold text-slate-400 mb-2">pH Level</label>
                            <input
                              type="number"
                              step="0.1"
                              className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-emerald-500"
                            ...
                            <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                              <div>
                                ...
                                <div className="flex items-center gap-4 mt-2">
                                  ...
                                  <h3 className="text-5xl font-black tracking-tight">{recommendation.crop}</h3>
                                </div>
                               ...
                               <div className="bg-white/20 p-4 rounded-2xl backdrop-blur-md border border-white/20">
                                 <button 
                                   ...
                                   className="flex items-center gap-2 bg-white text-emerald-900 px-4 py-2 rounded-xl font-bold hover:bg-emerald-100 transition-colors"
                                 >
                          ...
                          <div className="bg-[#064e3b]/40 rounded-3xl p-8 border border-emerald-900/30">
                            ...
                            <div className="grid md:grid-cols-2 gap-8">
                                <div className="space-y-4">
                                    <div className="p-4 bg-[#042f2e]/60 rounded-2xl border border-emerald-900/20">
                                        <h5 className="text-xs font-bold text-slate-500 uppercase tracking-widest">Expected Price</h5>
                                        <p className="text-xl font-black text-white mt-1">{recommendation.market.price}</p>
                                    </div>
                                    <div className="p-4 bg-[#042f2e]/60 rounded-2xl border border-emerald-900/20">
                                        <h5 className="text-xs font-bold text-slate-500 uppercase tracking-widest">Market Trend</h5>
                                        <p className="text-lg font-bold text-emerald-400 mt-1">{recommendation.market.trend}</p>
                                    </div>
                                </div>
                                <div className="h-40">
                                    <ResponsiveContainer width="100%" height="100%">
                                        <BarChart data={[
                                            { name: 'Last Month', price: 80 },
                                            { name: 'This Month', price: 95 },
                                            { name: 'Predicted', price: 110 }
                                        ]}>
                                    ...
                                    <p className="text-center text-[10px] text-slate-500 mt-2 font-bold uppercase tracking-widest">Projected Value Growth</p>
                                </div>
                            </div>
                          </div>
                      ...
                      <div className="space-y-8">
                        <div>
                          <div className="flex justify-between items-end mb-2">
                            ...
                            <span className="text-lg font-black text-white">{formData.humidity}%</span>
                          </div>
                          <div className="w-full h-3 bg-slate-950 rounded-full overflow-hidden">
                            <motion.div
                              initial={{ width: 0 }}
                              animate={{ width: `${formData.humidity}%` }}
                              className="h-full bg-blue-500"
                            />
                          </div>
                          <p className="text-xs text-slate-500 mt-2">Optimal range: 60% - 75% (Currently based on humidity)</p>
                        </div>
